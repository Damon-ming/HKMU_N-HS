...
from schemas.request import BaseBizRequest, BizBaseRequest
from schemas.response import BaseBizSuccessData,BaseBizFailedData

# ================= 1. 文件上传请求 =================
class FileUploadRequest(BaseBizRequest):
    """文件上传请求：继承全局基类，补充文件特有参数"""
    description: Optional[str] = Field(None, description="文件描述或备注")
    category: str = Field(..., description="文件分类（如：合同、发票）")
