...
export const ChatChatroom: React.FC = () => {
  const { input, setInput, fileName, uploading, uploadError, messages, send, handleFileChange } = useChatRoomController()
  return <main className={`feature-chatroom ${messages.length ? 'has-messages' : 'is-empty'}`}>
    ...
    <section className="feature-composer-area">
      ...
      <footer className="feature-composer"><label aria-label="上传图片">＋<input hidden accept="image/*" type="file" onChange={e => handleFileChange(e.target.files?.[0])} /></label><textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send() } }} placeholder={uploading ? '正在上传图片...' : fileName || '输入消息...'} disabled={uploading} /><button onClick={send} aria-label="发送消息" disabled={uploading}>↑</button></footer>
